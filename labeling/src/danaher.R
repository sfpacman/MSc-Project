...
all_type_expr_table <- all_type_expr %>% group_by(Cluster,Signature) %>% summarise(Barcode_count=sum(n_distinct(Barcode)), total_all_gene_Exp= round(sum(as.numeric(Expression)),3), avg_non_zero=round(mean(as.numeric(Expression)),3) , SD= round(sd(as.numeric(Expression)),3) )
all_type_expr_table<-merge(all_type_expr_table,cluster_count,by="Cluster")
all_type_expr_table<- cbind(all_type_expr_table,precent_count =round(all_type_expr_table$Barcode_count/all_type_expr_table$cell_total_count *100,3))
